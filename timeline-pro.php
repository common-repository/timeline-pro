...
function timeline_pro_add_shortcode_column( $columns ) {
    return array_merge( $columns, 
        array( 'shortcode' => __( 'Shortcode') ) );
}
add_filter( 'manage_timeline_pro_posts_columns' , 'timeline_pro_add_shortcode_column' );



add_filter( 'gettext', 'timeline_pro_add_change_publish_button', 10, 2 );

function timeline_pro_add_change_publish_button( $translation, $text ) {
if ( 'timeline_pro' == get_post_type())
if ( $text == 'Publish' )
    return 'Publish Accordion';
...
function timeline_pro_add_posts_shortcode_display( $column, $post_id ) {
    if ($column == 'shortcode'){
		?>
    <input style="background:#bfefff" type="text" onClick="this.select();" value="[timeline_pro <?php echo 'id=&quot;'.$post_id.'&quot;';?>]" /><br />
    <textarea cols="50" rows="1" style="background:#bfefff" onClick="this.select();" ><?php echo '<?php echo do_shortcode("[timeline_pro id='; echo "'".$post_id."']"; echo '"); ?>'; ?></textarea>
        <?php		
		
    }
}
add_action( 'manage_timeline_pro_posts_custom_column' , 'timeline_pro_add_posts_shortcode_display', 10, 2 );
